...
make -j$(nproc --all) O=/workspace/out ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- LLVM=1 LLVM_IAS=1 CC="ccache clang" $DEFCONFIG
if [ -v COMPILEDB ]; then
    compiledb -o /workspace/out/compile_commands.json make -j$(nproc --all) O=/workspace/out ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- LLVM=1 LLVM_IAS=1 CC="ccache clang"
else
    make -j$(nproc --all) O=/workspace/out ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- LLVM=1 LLVM_IAS=1 CC="ccache clang"
fi
...
echo "[+] Successfully built kernel for $DEFCONFIG"
